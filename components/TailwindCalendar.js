import React, {memo, useRef, useState, useEffect, createContext, useContext} from "react"
...
function App(){
  ...
  const [fullDate, setFullDate] = useState(moment().format("dddd, DD/MMMM/YYYY"))
...
const handleDateChange = (e)=>{
  ...
  useEffect(()=>{
    ...
    for(let i=1; i<=12; i++){
      ...
      obj.maxDays = date.endOf("month").format("D") - 0;
      obj.startDayFrom = date.startOf("month").day() - 0;
  ...
  return (
   ...
   {x}
    ...
    <IconButton sx={{ml: "auto"}} onClick={darkMode.toggleDarkMode}>
     {(darkMode.mode==="dark") ? <LightMode/> : <DarkMode />}
    </IconButton>
    </div>
    <div className="text-3xl text-center text-red-700 mb-2 uppercase">
    {yearRef.current} Calendar
    </div>
    <div className="sticky top-0 shadow-lg bg-red-700 text-gray-100 dark:text-gray-400 backdrop-blur-[2px] text-center text-xl py-2 bg-opacity-60 capitalize mb-2">
    {fullDate}
    </div>
    <div>
    {(loading===true) && (
...
function MyApp(){
  ...
  return (
    <ThemeProvider theme={theme}>
      <DarkContext.Provider value={obj}>
        <App />
